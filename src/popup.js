...
async function callChatGPT(messages, callback, onDone) {
  ...
  try {
    ...
    callback(
      'Please add your Open AI API key to the settings of this Chrome Extension.'
    );
    onDone();
    return;
  }
    ...
    try {
      ...
      // In progress
        ...
        options = {
          onProgress: () =>
            callback(
              'Processing your code changes. Number of prompts left to send: ' +
                iterations +
                '. Stay tuned...'
            ),
        };
...
async function reviewPR(diffPath, context, title) {
  ...
  promptArray.push(`The change has the following title: ${title}.
    ...
    - Check out the duplicated code in files.
  ...
  console.log('callChatGPT promptArray', promptArray.join('\n'));
